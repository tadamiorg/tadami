...
fun NavGraphBuilder.discoverNavGraph(navController: NavHostController) {
    navigation(
        ...
        composable(route = "${DiscoverRoutes.SEARCH}/{sourceId}?basequery={baseQuery}", arguments = listOf(
            navArgument("sourceId") { type = NavType.StringType },
            ...
            SearchScreen(navController = navController, baseQuery = it.arguments?.getString("baseQuery"))
        }
